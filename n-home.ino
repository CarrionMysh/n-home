...
byte verify_string() {                 //верификация ввода команды с Serial_pc
  ...
  if (flag==false) return (2);            //если в 1..2 байтах не встретился id из доступных
  //проверка секции командыне проверяется, если что не так - слэйв вернет unknow command, конец пакета '%'  так же не проверяем, т.к. вручную его вводить не будем
}
